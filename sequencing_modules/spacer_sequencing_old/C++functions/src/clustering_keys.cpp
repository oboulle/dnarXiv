...
void reconstruction(vector<string> SEQ, string SPACER, int FRAG_size, string output_dir) {
	...
	map<int, int> kmer_keys;
	map<char, int> nucleotide_to_int = {{'A', 0}, {'C', 1}, {'G', 2}, {'T', 3}};

	//divide each fragments into kmers, an unique int key is associated to each kmer
	// ACGT -> 0*4^3 + 1*4^2 + 2*4^1 + 3*4^0 -> key = 27
	for(const string& fragment : FRAG) {
		//cout << fragment << endl;
		int key = 0;
		char nucleotide;
		char removed_nucleotide;

		//compute the key for the first kmer
		for(int i=len_spacer; i<len_spacer+WS;i++){
			nucleotide = fragment[i];
			key = 4 * key + nucleotide_to_int[nucleotide];
		}
		kmer_keys[key]++;
		//cout << endl;

		//compute keys for the following kmers
		//each kmer is the last kmer with one nucleotide differing
		for(int i=len_spacer+WS; i<fragment.size()-len_spacer;i++){
			removed_nucleotide = fragment[i-WS];

			key = key - pow(4, WS-1)*nucleotide_to_int[removed_nucleotide];
			//cout << " - " << removed_nucleotide << " " << key << endl;

			nucleotide = fragment[i];
			key = 4 * key + nucleotide_to_int[nucleotide];
			//cout << nucleotide << " " << key << endl;
			kmer_keys[key]++;
		}
	}
	int SOLID_THRESHOLD = 5;
	//assign a set of kmer to each fragment
	vector<vector<int>> list_kmer_key;
	for(const string& fragment : FRAG) {
		set<int> frag_set;
		...
		for(int j=len_spacer; j<end_range; j++) {
			...
			if(kmer_keys[key] >= SOLID_THRESHOLD){
				frag_set.insert(key);
			}
			//cout << endl;
		...
		vector<int> frag_vector(frag_set.size());
		copy(frag_set.begin(), frag_set.end(), frag_vector.begin());
		list_kmer_key.push_back(frag_vector);
	}
	...
	cout << "construct_fragment_graph " << list_kmer_key.size() << endl;
	Graph G(len_FRAG);
	//int progress = -1;
	for(int i=0; i<len_FRAG; i++) {
		...
		for(int j=i+1; j<len_FRAG; j++) {
			...
			while(index_a < size_a and index_b < size_b) {
				...
				int diff = list_kmer_key[i][index_a] - list_kmer_key[j][index_b];
				if(diff < 0) {
					index_a++;
				} else if(diff > 0) {
					index_b++;
